...
+ (id)notifyHUDWithView:(UIView *)view text:(NSString *)text frame:(CGRect) frame {
    return [[self alloc] initWithView:view text:text frame:frame];
}
...
{
    return [[self alloc] initWithImage:image text:text frame:frame];
}
...
- (id)initWithView:(UIView *)view text:(NSString *)text frame:(CGRect) frame{
    if ((self = [self initWithFrame:frame]))
    {
        self.iconView = [self configureViewForCentering:view];
        self.text = text;
        [self setupViewRecalculateHeight:NO];
    }
    return self;
}
...
{
    if ((self = [self initWithFrame:CGRectMake(0, 0, 250.0f, kBDKNotifyHUDDefaultHeight)]))
    {
        self.iconView = [self configureViewForCentering:[[UIView alloc]init]];
        self.text = text;
        [self setupView];
    }
    return self;
...
-(void) setupViewRecalculateHeight:(BOOL) recalculate
{
...
- (void)setupView {
    [self setupViewRecalculateHeight:YES];
}

-(void) centerHUD:(UIView*) superView
{
    self.center = CGPointMake(superView.center.x, superView.center.y - self.textLabel.frame.size.height/2.5f);
}

-(void)presentWithSpeed:(CGFloat) speed onCompletion:(void (^)(void))completion
{
    self.isAnimating = YES;
    [UIView animateWithDuration:speed animations:^{[self setCurrentOpacity:self.destinationOpacity];} completion:
     ^(BOOL finished)
     {
         if (finished)
         {
...
{
    [self fadeAfter:0.1f speed:speed completion:completion];
}
...
- (void)recalculateHeight {
    ...
    frame.size.height = CGRectGetMaxY(self.textLabel.frame) + kBDKNotifyHUDDefaultPadding + self.iconView.frame.size.height;
